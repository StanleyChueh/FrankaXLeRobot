...
class DataRecorderNode(object):
    ...
    def convert_to_dataset_format(self, return_paths_only=True):
        """
        If return_paths_only=True: returns eef_position, eef_orientation, gripper_width, list_of_front_paths, list_of_rear_paths, list_of_agent_paths
        ...
        if return_paths_only:
            return eef_position, eef_orientation, gripper_width, front_paths, rear_paths, agent_paths
        ...
        return eef_position, eef_orientation, gripper_width, np.array(view1_frames), np.array(view2_frames), np.array(view3_frames)
    ...
    try:
        ...
        while episode_index < args.num_episodes and not recorder.exit_all:
            ...
            # 如果有錄到數據 (正常完成或按右鍵)
                ...
                eef_position, eef_orientation, gripper_width, front_paths, rear_paths, agent_paths = recorder.convert_to_dataset_format(return_paths_only=True)

                if front_paths is not None and len(front_paths) > 0:
                    try:
                        dataset.add_episode(episode_index, eef_position, eef_orientation, gripper_width, front_paths, rear_paths, agent_paths)
