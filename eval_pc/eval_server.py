...
class ACTInferenceServer:
    def __init__(self, ckpt_path, host='0.0.0.0', port=5001, device='cuda', eval_freq=None):
        ...
        self.policy = ACTPolicy.from_pretrained(ckpt_path, local_files_only=True)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"推論完成: {np.round(action_np[7],6)} | 總頻率: {total_freq:.2f} Hz | 純推論頻率: {infer_freq:.2f} Hz | 資料處理耗時: {proc_interval:.4f} s | 傳輸速率: {xfer_mbps:.2f} MB/s")
